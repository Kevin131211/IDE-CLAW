...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    // 复用已有provider或创建新的
    if (_providerCache.containsKey(widget.sessionId)) {
        ...
        appBar: AppBar(
          ...
          actions: [
            Consumer<MessageProvider>(
              builder: (_, p, __) => _buildStatusIndicator(p.wsStatus),
            ),
  ...
  Widget _buildQuickCommands() {
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          children: [
            _quickBtn('📸 截图', 'screenshot'),
            const SizedBox(width: 6),
            _quickBtn('▶️ 继续优化', 'continue_opt'),
            const SizedBox(width: 6),
            _quickBtn('⏹️ 停止优化', 'stop_opt'),
            const SizedBox(width: 6),
            _quickBtn('📊 状态', 'get_status'),
          ],
        ),
      ),
    );
  ...
  void _startDownload(PushMessage msg) async {
            ...
            if (msg.isStatus)
              Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 4),
                  Text('状态更新', style: TextStyle(fontSize: 11, color: Theme.of(context).colorScheme.primary)),
                ],
              ),
            if (msg.isScreenshot)
              Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.photo_camera, size: 14, color: Theme.of(context).colorScheme.primary),
                  const SizedBox(width: 4),
                  Text('截图', style: TextStyle(fontSize: 11, color: Theme.of(context).colorScheme.primary)),
                ],
              ),
            ...
            if (msg.isFile && _isImageFile(msg.fileName)) ...[
              if (msg.caption.isNotEmpty)
                Padding(
                  ...
                  child: Text(msg.caption.replaceAll(r'\n', '\n'), style: const TextStyle(fontSize: 14)),
                ),
