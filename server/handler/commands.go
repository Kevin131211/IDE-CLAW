...
func CreateCommand(db *store.DB, hub *ws.Hub) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 通知 PC 端有新指令（dialog.py / cascade 监听这个）
		hub.BroadcastToSession(req.SessionID, map[string]interface{}{
			"type": "command",
			"data": cmd,
		})

		// 同时以 message 类型广播：让其他客户端（手机/桌面）实时看到这条消息。
		// 客户端 ws_service 用 _seenIds 配合 sendCommand 写的 'cmd_<id>' 去重。
		// 不广播这条会导致一端发的消息另一端要等 15s poll 才看到。
		hub.BroadcastToSession(req.SessionID, map[string]interface{}{
			"type": "message",
			"data": msg,
		})
